...
func runGet(ctx context.Context, cmd *base.Command, args []string) {
	...
	for _, pkg := range downloadPaths(args) {
		download(ld, ctx, pkg, nil, &stk, mode)
	}
	base.ExitIfErrors()
	...
	pkgs := load.PackagesAndErrors(ld, ctx, load.PackageOpts{}, args)
	...
	work.InstallPackages(ld, ctx, args, pkgs)
}
...
func download(ld *modload.Loader, ctx context.Context, arg string, parent *load.Package, stk *load.ImportStack, mode int) {
	...
	load1 := func(path string, mode int) *load.Package {
		if parent == nil {
			...
			return load.LoadPackage(ld, ctx, load.PackageOpts{}, path, base.Cwd(), stk, nil, mode)
		}
		p, err := load.LoadImport(ld, ctx, load.PackageOpts{}, path, parent.Dir, parent, stk, nil, mode|load.ResolveModule)
		if err != nil {
			base.Errorf("%s", err)
		}
		return p
	...
	for _, p := range pkgs {
		if *getFix {
			...
			p = load.ReloadPackageNoFlags(ld, arg, stk)
			if p.Error != nil {
				base.Errorf("%s", p.Error)
				return
			}
		...
		for i, path := range imports {
			...
			if i >= len(p.Imports) {
				path = load.ResolveImportPath(ld, p, path)
			}
			download(ld, ctx, path, p, stk, 0)
		}
